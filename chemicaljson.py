...
class Elements(BaseModel):
    ...
    number: List[int] = Field(
        ...,
        description=
        "Required list of atomic numbers for the atoms in this file.")
...
class Coords(BaseModel):
    ...
    field3d: List[float] = Field(
        ...
        alias="3d",
        description=
        "List of 3d Cartesian coordinates (in Angstrom) for the atoms [ x, y, z, x, y, z, ... ]"
    )
    field3dFractional: Optional[List[float]] = Field(
        None,
        alias="3dFractional",
        description=
        "Optional list of 3d fractional coordinates for the atoms [ x, y, z, x, y, z, ... ]"
    )
    field3dSets: Optional[List[List[float]]] = Field(
        None,
        alias="3dSets",
        description=
        "Optional list of lists of 3d Cartesian coordinates (in Angstrom) for the atoms [ [x, y, z], [x, y, z], ... ]"
    )
...
class Atoms(BaseModel):
    ...
    labels: Optional[List[str]] = Field(
        None,
        description=
        "Optional list of custom labels for atoms (e.g., 'R' / 'S' or '0.12', etc.)"
    )
    layer: Optional[List[int]] = Field(
        None,
        description=
        "Optional list of layer numbers for the atoms (generally just 1 for most molecules)."
    )
...
class BasisSet(BaseModel):
    ...
    shellTypes: List[int] = Field(
        ...,
        description=
        "List of shell types for the basis functions (l-value, so s=0, p=1, d=2, etc.)."
    )
...
class Orbitals(BaseModel):
    ...
    moCoefficients: Optional[List[float]] = Field(
        None,
        description=
        "List of coefficients (flattened) for restricted molecular orbitals, i.e., alpha=beta (requires BasisSet to be present)."
    )
    alphaCoefficients: Optional[List[float]] = Field(
        None,
        description=
        "List of coefficients (flattened) for alpha open-shell orbitals, (requires BasisSet to be present)."
    )
    betaCoefficients: Optional[List[float]] = Field(
        None,
        description=
        "List of coefficients (flattened) for beta open-shell orbitals, (requires BasisSet to be present)."
    )
...
class Electronic(BaseModel):
    ...
    rotation: Optional[List[float]] = Field(
        None,
        description=
        "Optional list of rotation angles for the CD spectra (in degrees)")
...
class Nmr(BaseModel):
    ...
    shifts: List[float] = Field(
        ...,
        description=
        "List of absolute chemical shifts for the NMR spectra (in ppm)")
...
class Properties(BaseModel):
    ...
    totalSpinMultiplicity: Optional[int] = Field(
        1,
        description=
        "Total spin multiplicity of the system (2S+1, e.g., 1, 2, 3, etc.). If omitted, assume to be 1 (singlet)"
    )
...
class UnitCell(BaseModel):
    ...
    cellVectors: Optional[List[float]] = Field(
        ...
        description=
        "Optional list of cell vectors (in Angstrom): [ x1, y1, z1, x2, y2, z2, ... ]"
    )
...
class Vibrations(BaseModel):
    ...
    eigenVectors: List[List[float]] = Field(
        ...,
        description=
        "List of eigenvectors (displacements in Angstroms) for the vibrations."
    )
...
class CJSONModel(BaseModel):
    ...
    chemicalJson: int = Field(
        1,
        description=
        "Version number of the Chemical JSON format. Currently 1. Only changed for backwards-incompatible changes to the schema."
    )
    atoms: Atoms = Field(
    ...
    properties: Optional[Properties] = Field(
        None,
        description=
        "Optional free-form Properties, including total charge and total spin multiplicity."
    )
    inputParameters: Optional[InputParameters] = Field(
        None,
        description=
        "Optional InputParameters object, including calculation metadata such as basis set, job type, etc."
    )
    metadata: Optional[Metadata] = Field(
        None,
        description=
        "Optional metadata object, including calculation details which do not directly impact results"
    )
    partialCharges: Optional[PartialCharges] = Field(
        None,
        description=
        "Optional PartialCharges object. Includes atomic partial charges and population analysis."
    )
